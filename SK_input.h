...
// My version of strcpy_s which needs these parameters(destString, sourceString)
// My version assumes MAX_STRING_LEN as a globally defined constant
void myStrCpy(char dest[], const char source[]) {
	int i = 0;
	while ((source[i] != '\0') && (i < MAX_STRING_LEN - 1)) {
		dest[i] = source[i];
		i++;
	}
	dest[i] = '\0';
